...
public class Drivetrain extends SubsystemBase {
  ...
  public Drivetrain() {
    ...
    if (!RobotBase.isReal()) {
      driveSim = new DifferentialDrivetrainSim(DriveConstants.DRIVETRAIN_PLANT, DriveConstants.MOTOR,
          DriveConstants.GEAR_RATIO, DriveConstants.TRACK_WIDTH, DriveConstants.WHEEL_DIAMETER / 2,
          DriveConstants.MEASUREMENT_STD_DEVS);
    }
    drivePose = new DifferentialDrivePoseEstimator(driveKinematics, getGyroAngle(), getLeftPosition(),
        getRightPosition(), new Pose2d());
  }
  ...
  public double getLeftPosition() {
    ...
    if (RobotBase.isReal()) {
      return leftMotor1.getEncoder().getPosition() / DriveConstants.GEAR_RATIO * DriveConstants.WHEEL_DIAMETER
          * Math.PI;
    } else {
  ...
  public double getRightPosition() {
    ...
    if (RobotBase.isReal()) {
      return rightMotor1.getEncoder().getPosition() / DriveConstants.GEAR_RATIO * DriveConstants.WHEEL_DIAMETER
          * Math.PI;
    } else {
